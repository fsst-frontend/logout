<script setup>
import { ref } from 'vue'
import { getMyTraderRebateRecordList } from '@/api/invite.js'
...
function switchTab(tab) {
  ...
  getMyTraderRebateRecordList(params).then((res) => {
    users.value = (res.data.record_list || []).map((item) => {
      ...
      item.tradeDetails = {
        ...
        rebateRate: `${item.rebate_ratio * 100}%`,
        rebateAmount: `${item.rebate_amount} USD`,
      }
      return item
    })
  }).finally(() => {
        ...
        <div v-else class="card-body">
          <div v-for="(user, index) in users" :key="index">
            <div class="card-header">
              <div class="flex">
                <div><img :src="user.rebate_provider_avator" class="rd-30px h-40px w-40px"></div>
                <div class="ml-15px">
                  <span class="user-id">{{ user.rebate_provider_uid }}</span>
              ...
              <div class="text-[12px] color-[#16C000] leading-[20px] px-8px rounded-4px bg-[#EBF7ED] h-20px w-52px">
                {{ user.status ? $t('rebate.settled') : $t('rebate.pendingSettlement') }}
              </div>
            </div>
            ...
            <!-- <div class="info-row">
              <span class="info-label">{{ $t('rebate.rebateRate') }}</span>
